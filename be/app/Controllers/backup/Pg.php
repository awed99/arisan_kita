...
class Pg extends BaseController
{
    ...
    public function postGet_pg_list()
    {
        ...
        $data = $db->table('pg_list_setting')->where('id_user', $id_user)->get()->getResult();
    ...
    public function postSet_pg_list()
    {
        ...
        foreach ($dataPost as $dtPost) {
          if ($dtPost['api_key']) {
            ...
            $_data = $db->table('pg_list_setting')->where('id_user', $id_user)->get()->getRow();
            
            if ($_data) {
              $updateX = $db->table('pg_list_setting')->where('id_user', $id_user)->ignore()->update($update);
            } else {
