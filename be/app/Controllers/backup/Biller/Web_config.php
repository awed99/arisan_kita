...
class Web_config extends BaseController
{
	...
	public function postUpload_image() {
		...
		$isset = $db->table('web_config')->where('id_user', 0)->get()->getRow();
		if ($isset) {
			$db->table('web_config')->where('id_user', 0)->update($update);
		} else {
	...
	public function postView() {
        ...
        $data = $db->table('web_config')->where('id_user', 0)->get()->getRow();
        $db->close();
	...
	public function postSave() {
        ...
        $data = $db->table('web_config')->where('id_user', 0)->update($update);
        $db->close();
