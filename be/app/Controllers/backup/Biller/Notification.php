...
class Notification extends BaseController
{
	...
	public function postGet_my_notif() {
        ...
        $notif = $db->table('app_notifications')->where('id_user', 0)->orWhere('id_user', -1)->orderBy('id', 'desc')->get()->getResult();
        $db->close();
	...
	public function postRead() {
        ...
        $db->table('app_notifications')->where('id_user', 0)->orWhere('id_user', -1)->where('id', $dataPost['id'])->ignore()->update($notif);
        $notif = $db->table('app_notifications')->where('id_user', 0)->orWhere('id_user', -1)->orderBy('id', 'desc')->get()->getResult();
        $db->close();
	...
	public function postRead_all() {
        ...
        $notif['is_read'] = 1;
        $notif['read_date'] = date('Y-m-d H:i:s');
        $db->table('app_notifications')->where('id_user', 0)->orWhere('id_user', -1)->ignore()->update($notif);
        $notif = $db->table('app_notifications')->where('id_user', 0)->orWhere('id_user', -1)->orderBy('id', 'desc')->get()->getResult();
        $db->close();
